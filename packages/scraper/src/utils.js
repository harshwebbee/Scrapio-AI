...
export function isInternalUrl(candidate, root) {
    return new URL(candidate).hostname === new URL(root).hostname;
}
export function pageSlug(pageUrl) {
...
export function safeFilename(assetUrl) {
...
export function classifyAsset(assetUrl) {
...
export function assetFolder(type) {
    if (type === "image")
        return "images";
    if (type === "video")
        return "videos";
...
export function chunkContent(pagePath, content, startIndex) {
    ...
    while (cursor < words.length) {
        ...
        if (cursor + chunkSize >= words.length)
            break;
