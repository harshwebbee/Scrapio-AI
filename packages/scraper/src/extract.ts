...
import type { ExtractedAsset, ExtractedLink, ExtractedPage, PageMetadata } from "./types.js";
...
export function extractPage(html: string, pageUrl: string, rootUrl: string): ExtractedPage {
...
export function discoverLinks(html: string, pageUrl: string, rootUrl: string): string[] {
  const $ = cheerio.load(html);
  const discovered = new Set<string>();
  $("a[href]").each((_, el) => {
    const normalized = normalizeUrl($(el).attr("href") ?? "", pageUrl);
    if (normalized && isInternalUrl(normalized, rootUrl)) discovered.add(normalized);
  });
  return [...discovered];
}

function extractMetadata($: cheerio.CheerioAPI): PageMetadata {
  const content = (selector: string) => $(selector).attr("content")?.trim() ?? "";
  return {
...
function extractLinks($: cheerio.CheerioAPI, pageUrl: string, rootUrl: string): ExtractedLink[] {
...
function extractAssets($: cheerio.CheerioAPI, pageUrl: string): ExtractedAsset[] {
...
function getReadableHtml(html: string, pageUrl: string): string | null {
  try {
...
export function slugForPage(pageUrl: string): string {
